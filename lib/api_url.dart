...
Future<String> userLogin(phonenumber) async {
  final response = await http.get(Uri.parse(
      'http://192.168.1.20:8080/SpringMVCTemplate/nexusapp/generateOtp?phonenumber=$phonenumber'));
...
Future<String> userRoleApi(phonenumber) async {
  final response = await http.get(Uri.parse(
      'http://192.168.1.20:8080/SpringMVCTemplate/nexusapp/getRole?phonenumber=$phonenumber'));
...
Future<String> Verifyapi(num1, num2, num3, num4, num5, phonenumber) async {
  ...
  final response = await http.get(Uri.parse(
      "http://192.168.1.20:8080/SpringMVCTemplate/nexusapp/validateOtp?otp=$otp&phonenumber=$phonenumber"));
...
Future<String> flatApi(
    String name, mobilenumber, relationship, floor, flatno) async {
  Map data = {
    'name': name,
...
visitorApi(String typeOfVisitor, apartmentName, blockName, name, mobile,
    expectedTime, outTime, outDate, elapsedTime, File image) async {
  Map data = {
    'name': name,
    'mobile': mobile,
...
// <---users module starts---> //
//users api request
userApi(firstname, lastname, role, mobilenumber, email, venturename, block,
    flatno, gender, File image) async {
  Map details = {
    'firstname': firstname,
    'lastname': lastname,
    'number': mobilenumber,
...
// to send data to the server
Future<String> noticeApi(
    title, description, startdate, enddate, createdby) async {
